...
export const login = async (inputValues,dispatch,router) =>{
    try{
        const res = await axios.post(`http://62.113.96.105:5001/api/auth/login`,{
...
export const setUser = async (functions) =>{
    try{
        ...
        const res = await axios.post(`http://62.113.96.105:5001/api/auth/setUser`,{
...
export const auth = async (dispatch) =>{
    try{
        const res = await axios.post(`http://62.113.96.105:5001/api/user/auth`, {},{
            withCredentials: true
        })
...
export const logout = async (router,dispatch) =>{
    try{
        const res = await axios.post(`http://62.113.96.105:5001/api/auth/logout`,{}, {withCredentials: true})
        dispatch(setProfile({name: null,surname: null,email: null,user_id: null,phone: null,role: null,token: null}))
        router.replace('/auth')
    }
    catch(err){
